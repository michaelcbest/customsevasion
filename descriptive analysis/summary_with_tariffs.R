...
product_year_trade <- hs12_value[, uniqueN(`Commodity Code`), by=Period]
product_year_trade <- rename(product_year_trade, Products_trade = V1)

pair_year_trade <- unique(setDT(hs12_value), by = c("Importer", "Exporter", "Period"))
...
year_product_trade <- hs12_value[, uniqueN(`Period`), by=`Commodity Code`]
year_product_trade <- rename(year_product_trade, Years_trade = V1)

pair_product_trade <- unique(setDT(hs12_value), by = c("Importer", "Exporter", "Commodity Code"))
...
product_pair_trade <- hs12_value[, uniqueN(`Commodity Code`), by = c("Importer", "Exporter")]
product_pair_trade <- rename(product_pair_trade, Products_trade = V1)

year_pair_trade <- hs12_value[, uniqueN(`Period`), by = c("Importer", "Exporter")]
...
trade <- hs12_value[, .N, by = "Importer"]
...
trade <- hs12_value[Period!=2016 & `Commodity Code`!="999999", ]
...
rm(hs12_all_tariffs, hs12_value, matches, tariffs, trade)
...
hs12_all_tariffs <- hs12_all_tariffs[, .(Year, ProductCode, Importer, Exporter, Log_gap)]

hs12_all_tariffs$Year <- as.Date(hs12_all_tariffs$Year, "%Y")
hs12_all_tariffs$Year <- floor_date(hs12_all_tariffs$Year,"year")

Years <- hs12_all_tariffs[, .(mean = as.double(mean(Log_gap)),
...
#Across products?
products <- hs12_all_tariffs[, .(mean = as.double(mean(Log_gap)),
                           median = as.double(median(Log_gap)),
                           p25 = as.double(quantile(Log_gap,.25)),
                           p75 = as.double(quantile(Log_gap,.75))
),
...
#Across countries?
countries <- hs12_all_tariffs[, .(mean = as.double(mean(Log_gap)),
                            median = as.double(median(Log_gap)),
                            p25 = as.double(quantile(Log_gap,.25)),
                            p75 = as.double(quantile(Log_gap,.75))
),
...
hs12_all_tariffs <- hs12_all_tariffs[, .(Year, ProductCode, `Reporter_ISO_N`, `Partner Code`, Log_gap)]
...
hs12_all_tariffs <- hs12_all_tariffs[, .(Year, Log_gap, Year.f, Products.f, Pairs.f)]

reg <- felm(Log_gap ~ 1 | Year.f + Products.f + Pairs.f,
...
hs12_all_tariffs <- hs12_all_tariffs[, 
                    .(Year, ProductCode, `Reporter_ISO_N`, `Partner Code`, SimpleAverage)]
...
hs12_all_tariffs <- hs12_all_tariffs[, .(Year, ProductCode, Importer, Exporter, SimpleAverage, Log_gap)]
hs12_all_tariffs <- hs12_all_tariffs[!is.na(SimpleAverage)]

tariffs <- hs12_all_tariffs[, .(mean = mean(Log_gap)), by = SimpleAverage]
...
ggplot(tariffs, aes(SimpleAverage, value, colour=Legend, size=Legend)) +
  ...
  labs(title="Evasion Gap at Different Tariff Rates, 2012-2015") +
  background_grid(major = 'y', minor = "none") +
  labs(x="Tariff rate (%)", y="Value gap, log scale") +
...
tariffs <- tariffs[, .(mean = mean(Log_gap)), by = SimpleAverage]
...
simpreg = lm(Log_gap ~ SimpleAverage, data = hs12_all_tariffs)
summary(simpreg)
